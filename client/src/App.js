...
export default function App() {
   ...
   useEffect(() => {
      (async () => {
         const [o, r, c, m] = await Promise.all([
            db.get("orders"), db.get("reviews"),
            db.get("complaints"), db.get("menu"),
         ]);
         if (o) setOrders(o);
         if (r) setReviews(r);
   ...
   const saveOrders = useCallback(async (data) => { setOrders(data); await db.set("orders", data); }, []);
   const saveReviews = useCallback(async (data) => { setReviews(data); await db.set("reviews", data); }, []);
   const saveComplaints = useCallback(async (data) => { setComplaints(data); await db.set("complaints", data); }, []);
   const saveMenu = useCallback(async (data) => { setMenu(data); await db.set("menu", data); }, []);
...
function StudentApp({ orders, reviews, complaints, menu, saveOrders, saveReviews, saveComplaints, onExit }) {
   ...
   const G = "linear-gradient(90deg,#0A2E1C,#165C2E)";
   ...
   const generateAIResponse = async (complaint) => {
      ...
      try {
         const resp = await askAI(
            `You are a professional cafeteria manager at Landmark University's Campus Chop Hub. Write a brief, empathetic, professional response to a student's complaint. Be polite, acknowledge the issue, apologize where needed, and state corrective action. Keep it under 60 words.`,
